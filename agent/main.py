...
async def crewai_agent(input_data: RunAgentInput):
    ...
    try:
        ...
        async def event_generator():
            ...
            state_dict = input_data.state if isinstance(input_data.state, dict) else {}
            available_cash = state_dict.get("available_cash", 1000000)
            investment_summary = state_dict.get("investment_summary", {})
            investment_portfolio = state_dict.get("investment_portfolio", [])

            yield encoder.encode(
                StateSnapshotEvent(
                    type=EventType.STATE_SNAPSHOT, 
                    snapshot={
                        "available_cash": available_cash,        # Current cash balance
                        "investment_summary": investment_summary, # Previous analysis results
                        "investment_portfolio": investment_portfolio, # Current holdings
            ...
            state = AgentState(
                ...
                available_cash=available_cash,                              # Current cash
                investment_portfolio=investment_portfolio,                 # Current portfolio
...
def main():
    ...
    print(f"Starting Stock Portfolio Agent FastAPI server on port {port}...", flush=True)
